...
修:加上 `registration.updatefound` 和 `registration.waiting` 检测,**每次 push bump CACHE_VERSION 改 SW 源 byte** 才能触发 updatefound。详见 [06-pwa-hot-update.md](06-pwa-hot-update.md)。

### Safe-area-inset 必备

iPhone 刘海 / dynamic island 会盖顶栏。`viewport-fit=cover` + `env(safe-area-inset-top)` 才不被遮。drawer / progress bar / 顶栏都要。详见 [09-css-layout-traps.md](09-css-layout-traps.md) 坑 2。
...
桌面测时拖小窗口 → 缩到 < 864 → page 应该开始变小。如果不变,RO/resize listener 没 fire,见 [02-fit-width-zoom.md](02-fit-width-zoom.md)。
...
## 相关
- [02-fit-width-zoom.md](02-fit-width-zoom.md) — cozy 公式怎么平衡这些 quirks
- [06-pwa-hot-update.md](06-pwa-hot-update.md) — iOS PWA 更新检测
- [09-css-layout-traps.md](09-css-layout-traps.md) — safe-area / scrollbar 细节
