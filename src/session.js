...
import {
  SESSION_FILE, POSITION_DEBOUNCE_MS, POSITION_HEARTBEAT_MS,
  TRIVIAL_POSITION_Y_DELTA,
} from "./config.js";
...
// flushKeepalive 顺手带上。避免 OneDrive 版本历史被微动塞爆。
...
export function setPosition(itemId, position) {
  ...
  // trivial 判定:跟上次成功推到 OneDrive 的位置比,同页 + yFraction 微动 → 不调度 PUT。
  // 仍标 dirty,这样 flushKeepalive (tab hidden / close) 时会被一起带上。
  const ref = lastPushedPositions[itemId];
  const isTrivial = ref
    && ref.pageIndex === newPos.pageIndex
    && Math.abs(ref.yFraction - newPos.yFraction) < TRIVIAL_POSITION_Y_DELTA;
  if (isTrivial) {
    dirty = true;
