...
await f.offload();        // 移除本地副本（只删本地，云端保留）
await f.isKeptOffline();  // 本地有副本？（= 已留作离线）
store.localKeys();        // 已留作离线（=有本地副本）的文件名列表（批量标记 UI 用）
```
- **心智模型**：有本地副本 = "kept offline"。**无 LRU、无 pin flag、无 unpin、无 force**——只有「留一份」(`keepOffline`) 和「移除」(`offload`) 两个动词；中间那种「可被自动驱逐的 cache」态**不存在**（开了 / 下载了就留着，直到你显式 `offload`）。
- `keepOnOpen:true`（默认，见 §1）下 **`open` 即自动留本地**（= 自动 keepOffline）；流式消费 app 用 `keepOnOpen:false`（⚠TODO 未实现）则 `open` 过路不留，只有显式 `keepOffline` 才落地。
- `offload()` 只在「**clean ∧ 在线 ∧ 已登录 ∧ 云端仍有完整副本**」时执行，否则**保留**——dirty（未推）/ 离线 / 未登录 / 云端没了 一律不丢（那时本地可能是唯一一份好副本）。这条守卫在 `offload` 深模块内 enforce，UI 永不自己保证。
