...
reading.upsertItem({ id: docId, pageIndex, yFraction });  // 新增 / 整条原子替换
reading.deleteItem(docId);
reading.getItem(docId);                                   // 一条 | undefined
reading.listItems();                                      // 全部
```
- 用于：阅读位置表、笔架、任何"一堆小条目、跨设备合并、零冲突"的东西。
- **item 是原子的：只有 `upsertItem`（整条替换），没有 partial update。** 想改一个字段 = 取出整条、改、整条 upsert。换来的是合并简单 + 无中间态。
- 内部 pull-merge-push，**per-item last-write-wins**（每 item 一个时间戳，并发改不同 item 都不丢）。不丢、不静默覆盖。
...
await f.save(bytes);   // 已解锁 → 库自动加密；未解锁 → 抛 EncryptionLocked
await f.open();        // 已解锁 → 自动解密；未解锁 → 抛 EncryptionLocked

// 每文件独立密码（少见）：encrypted 给一个非交互取密码的函数（读内存，库永不弹框）
const g = store.file("x.ora", { isZip: true, encrypted: { password: () => myPw } });
```
- 加密对 `save`/`open` **透明**：你照常存原始 `bytes`，库负责打/拆容器。
- `ZipFile + encrypted`：`previewBlob` 一起加密进容器尾部，`getPreview` 解锁后仍可读（云端尾部 byte-range，不全量下载）。
- 解锁/弹窗/验证/重试**必须在 busy 遮罩之外**做（遮罩盖住密码框 = 死锁）。典型：捕获 `EncryptionLocked` → 在遮罩外 prompt → `store.encryption.unlock(pw)` → 重试。
...
很多 API 形状**逼你把对应 UI 做对**：
- flow 返回 `conflict` / `EncryptionLocked` / `invalid-cloud-bytes` → 你**必须**接冲突 sheet / 密码框 / 提示。不接 = 用户数据风险。
