...
```ts
await f.pin();         // 缓存到本地并常驻（离线常用的，不被自动清除）
f.unpin();            // 取消常驻
await f.evict();      // 清除本地缓存（只删本地副本，云端保留）
await f.isCached();   f.isPinned();
store.localKeys();    // 已缓存的文件名列表（批量标记 UI 用）
```
- `evict()` 只在「文件无未上传改动、在线、云端仍有副本、未常驻」时执行，否则保留——绝不丢未上传的改动。
