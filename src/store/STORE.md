...
const store = createStore({
  ...
  sevenZip,                                  // 选填：注入 7z 编解码器（加密用）。不注入 → 不支持加密（§5.0）
...
加密的**逻辑**（容器 / 层数 / 预览图加密 / KDF / salt / 验证器 / peek）全在库内。app 只负责两件库做不了的事：**注入 7z 引擎**（§5.0）+ **把密码输进来**（库非交互、永不弹框，§7）。

### 5.0 前提：注入 7z（体积大、按需 vendor）

唯一不随库走的是**重型 7z 引擎（wasm ~1.6MB）**——体积大，不该塞进每个 app 的 bundle，所以**由 app 注入**：
```ts
import { createSevenZip } from "./vendor/sevenzip-adapter.ts";   // app 侧：vendor 7z-wasm + 懒加载
const store = createStore({ provider, ui, encryptionSaltFileName: "vault.salt",
  sevenZip: createSevenZip({ wasmUrl: "./vendor/7z-wasm/7zz.wasm" }) });
```
- **不注入 `sevenZip` → 不支持加密**：`store.encryption` 在类型上就不存在，`encrypted:true`/`saveEncrypted` 抛清晰错误。（JRP 不加密 → 不 vendor、不注入，省那 1.6MB。）
- `sevenZip` 实现**应懒加载** wasm（首次加密/解密才拉，绝不在 boot）。
- 反观 **zip.js（容器机制，小）随库 ship**、**KDF/GCM 验证器走浏览器内置 WebCrypto**——都不用注入。
...
| 加密 | 自己写 zip/7z/容器 | 注入 `sevenZip`（§5.0）+ `encrypted:true`/`store.encryption`，逻辑库管 |
