...
- ⚠TODO **range / streaming（流式消费，配合 `keepOnOpen:false`）**：大媒体（RealHome glb / Background Radio 音频）不该整块 `open` 落本地。未来 `open` 会**路由 cache-or-remote**——本地有就读本地，没有就按 range 从云端流（`provider.downloadRange` 已具备），只显式 `keepOffline` 才整份落地。形状以后慢慢设计；现在 `keepOnOpen:false` 直接抛「未实现」。
