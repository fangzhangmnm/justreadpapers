...
- **透明本地缓存（IDB，`as-of v? / 2026-06-23`）**：collection 自动把整份 envelope（含各 item 的 `uat`）镜像进 IDB（键 `__collection__/<name>`，不与 file 路径撞）。
  - `init()` 先从本地 hydrate（`mergeFolders` 合并不替换、保留 uat）→ **离线可读 + reload/强杀续读**；再 pull-merge-push。**云端坏字节/空/离线绝不 wipe 本地**（`parseFolderBlob` 返 null 即忽略）。
  - 编辑后 coalesce 写本地（默认 400ms 防抖，避免高频 `setPosition` 每帧写 IDB）；`flush()` 与 `flushLocal()` 立即落。
  - **`flushLocal()`**：只写本地、不碰网络——卸载（pagehide/visibilitychange）兜底调它（云写在页面被杀时不可靠，本地连续 coalesce 写 + flushLocal 才是续读的可靠来源）。
  - **旧设备旧缓存不盖新**（A5）：合并仍是 per-item `uat`（=user-action-time）max-wins、forward-only；旧本地缓存的旧 uat 永远输给云端新 uat。⚠ uat 必须是用户动作时间，不是 save/sync/cache 时间（红线 B6）；boot/reconcile 期间别让杂散滚动盖新 uat（B7，app 侧 loading 门）。
  - **不传 `local`**（如纯测试）= 退回旧行为（纯内存+云，reload 丢）。`createStore` 默认注入 IDB local。
